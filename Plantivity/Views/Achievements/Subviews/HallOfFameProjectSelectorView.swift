...
struct HallOfFameProjectSelectorView: View {
    ...
    var selectedProjects: [Project] {
        ...
        for projectImage in achievementsData.projectImages {
            if projectImage.project != nil {
                projectList.append(projectImage.project!)
            }
        }
        return projectList
    }
                            ...
                            Button {
                                if selectedProjects.contains(project){
                                    if let index = selectedProjects.firstIndex(of: project) {
                                        achievementsData.projectImages.remove(at: index)
                                    }
                                } else {
                                    achievementsData.projectImages.append(ProjectImage(id: project.id ?? UUID(), project: project, x: 0, y: 0))
                                }
                            } label: {
                                ProjectHallOfFameListItem(project: project, selected: selectedProjects.contains(project))
                            }
                        }
